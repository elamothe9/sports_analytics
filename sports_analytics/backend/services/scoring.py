...
# ─────────────────────────────────────────────
# Data fetchers
# Slow external calls are memoized with short TTLs so repeated
# requests (and multiple endpoints) reuse the same league-wide data.
# ─────────────────────────────────────────────
...
def fetch_team_map() -> dict:
    """Fetch all MLB teams. Returns {team_id: team_name}."""
    url = f"{MLB_API_BASE}/teams"
    params = {"sportId": 1}
    res = requests.get(url, params=params, timeout=15)
    res.raise_for_status()
    return {
        t["id"]: t.get("name", "Unknown")
...
def score_player(
    ...
    # Optional prefetched data (routers batch-fetch these concurrently)
    splits: dict | None = None,
    h2h: dict | None = None,
) -> dict:
    year = datetime.now().year

    # Fetch player-specific data (unless prefetched by the caller)
...
def get_statcast_batter_cached(
    batter_id: int, persist: bool = True
) -> "pd.DataFrame":
